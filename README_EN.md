...
- `delete_note` - Delete one published note, irreversibly (required: note_id)
  - **Not registered by default**: the server needs `XHS_ENABLE_DELETE=1` before this tool appears at all, and the HTTP route `POST /api/v1/notes/delete` answers 403 until it is set
  - It takes a concrete `note_id` and nothing looser; ids come from listing tools such as `get_my_profile`
  - It draws on the same daily publish budget as publishing does
